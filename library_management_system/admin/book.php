...
if(isset($_POST["add_book"]))
{
	...
	if (isset($_FILES['book_pic']) && !empty($_FILES['book_pic']['name'])) {
		$img_name = $_FILES['book_pic']['name'];
		$img_type = $_FILES['book_pic']['type'];
		$tmp_name = $_FILES['book_pic']['tmp_name'];
		...
		if (in_array($img_ext, $extensions)) {
			...
			if (move_uploaded_file($tmp_name, "bookimg/" . $new_img_name)) {
				$formdata['book_pic'] = $new_img_name;	
		    }
		    else
		    {
        ...
        $message .= '<li>Please Select Book Image</li>';
	    }
	}
	...
	if($error == '')
	{
        ...
        (book_pic, book_category, book_author,  book_name, book_isbn_number, book_no_of_copy, book_status, book_added_on) 
        VALUES (:book_pic, :book_category, :book_author, :book_name, :book_isbn_number, :book_no_of_copy, :book_status, :book_added_on)
		";
        	...
        	<form method="post" enctype="multipart/form-data">
				...
				<div class="col-md-6">
        				<div class="mb-3">
        					...
        					<input type="file" name="book_pic" id="book_pic" class="form-control" />
        				</div>
        			</div>
        			<div class="col-md-6">
        				<div class="mb-3">
        	...
        	<table id="datatablesSimple">
        		...
        		if($statement->rowCount() > 0)
        		{
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				echo '
        				<tr>
						    <td><img src="bookimg/'.$row["book_pic"].'"
...
//echo '<img src="assets/bookimg/'.$row["book_pic"].'" class="img-thumbnail" alt="'.$row["book_name"].'" width="75" />';
//echo '<pre>' . print_r($row["book_pic"], true) . '</pre>'; 
if(isset($row["book_pic"])) {
    echo '<img src="bookimg/'.$row["book_pic"].'" class="img-thumbnail" alt="'.$row["book_name"].'" width="75" />';
} else {
